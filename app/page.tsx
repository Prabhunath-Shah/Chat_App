...
export default function ChatPage() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading, setMessages } = useChat({
    ...
    onError: (error) => {
      ...
      if ((error as any).response && typeof (error as any).response.json === "function") {
        ;(error as any).response
          ...
          .then((data: any) => {
            ...
            toast({
              title: "Error",
              description: `Failed to send message: ${data.error || data.details || errorMessage}`,
              variant: "destructive",
            })
          })
          .catch(() => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const systemMessage: Message = {
        id: `system-${Date.now()}`,
        role: "assistant",
        content: `📄 PDF "${file.name}" has been uploaded successfully! I can now answer questions about its content. The document contains ${Math.round(text.length / 1000)}k characters of text.`,
        timestamp: new Date(),
      }
  ...
  const handleRemovePdf = () => {
    ...
    const systemMessage: Message = {
      id: `system-${Date.now()}`,
      role: "assistant",
      content: "📄 PDF has been removed. You can now have a general conversation or upload a new PDF document.",
      timestamp: new Date(),
    }
